...
class QuantileGRU(nn.Module):
    def __init__(self, input_dim, hidden_sizes, num_steps, quantiles, dropout=0.0):
        super(QuantileGRU, self).__init__()
        ...
        for h_size in hidden_sizes:
            self.grus.append(nn.GRU(prev_dim, h_size, batch_first=True))
            prev_dim = h_size
...
def objective(trial):
    ...
    for m_start, m_end in zip(month_starts, month_ends):
        ...
        model = QuantileGRU(input_dim=X_train.shape[2], hidden_sizes=hidden_sizes, num_steps=Y_train.shape[1], quantiles=quantiles, dropout=dropout).to(device)
...
lookback = 25
batch_size = 16
dropout = 0.21530331797287366
hidden_sizes = [150]
quantiles = [0.90, 0.95, 0.97]
num_epochs = 100  
lr = 0.00015317781944601197
...
for m_start, m_end in zip(month_starts, month_ends):
    ...
    model = QuantileGRU(input_size, hidden_sizes, forecast_horizon, quantiles, dropout=dropout).to(device)
    optimizer = optim.RMSprop(model.parameters(), lr=lr)
