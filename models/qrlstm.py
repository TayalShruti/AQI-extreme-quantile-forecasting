...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x = self.inputs[idx]   # (lookback, num_features)
        y = self.outputs[idx]  # (forecast_horizon,)
        return (
...
class QuantileLSTM(nn.Module):
    def __init__(self, input_dim, hidden_sizes, num_steps, quantiles, dropout=0.0):
        ...
        for h_size in hidden_sizes:
            self.lstms.append(nn.LSTM(prev_dim, h_size, batch_first=True))
            prev_dim = h_size
        self.post_norm = nn.LayerNorm(hidden_sizes[-1]) 
        self.dropout = nn.Dropout(p=dropout) if dropout > 0 else nn.Identity()
        self.fc = nn.Linear(hidden_sizes[-1], num_steps * self.num_quantiles)

    def forward(self, x):
        h = x
        for i in range(self.num_layers):
            out, _ = self.lstms[i](h)
            h = out
        last = out[:, -1, :]
        last = self.post_norm(last)
        last = self.dropout(last)
        out = self.fc(last)  # [batch, num_steps * num_quantiles]
...
def objective(trial):
    ...
    for m_start, m_end in zip(month_starts, month_ends):
        ...
        model = QuantileLSTM(input_dim=X_train.shape[2], hidden_sizes=hidden_sizes, num_steps=Y_train.shape[1], quantiles=quantiles, dropout=dropout).to(device)
...
lookback = 40
batch_size = 32
dropout = 0.2779557536045263
hidden_sizes = [100]
quantiles = [0.90, 0.95, 0.97]
num_epochs = 100  
lr = 0.0004238937999435986
...
for m_start, m_end in zip(month_starts, month_ends):
    ...
    model = QuantileLSTM(input_size, hidden_sizes, forecast_horizon, quantiles, dropout=dropout).to(device)
    optimizer = optim.RMSprop(model.parameters(), lr=lr)
